...
class Report:
    ...
    def createRootResultFolder():
        ...
        elif XPParams.includeEmbedding and  XPParams.useAutoGeneratedDEP and  XPParams.useAutoGeneratedPOS and XPParams.useCrossValidation:
            Paths.rootResultFolder = os.path.join(Paths.rootResultFolder, 'Auto', 'Embedding')
        elif not XPParams.includeEmbedding and  XPParams.useAutoGeneratedDEP and  XPParams.useAutoGeneratedPOS and not XPParams.useCrossValidation:
            Paths.rootResultFolder = os.path.join(Paths.rootResultFolder, 'Auto', 'TestDataset')
        elif not XPParams.includeEmbedding and  XPParams.useAutoGeneratedDEP and  XPParams.useAutoGeneratedPOS and XPParams.useCrossValidation:
            Paths.rootResultFolder = os.path.join(Paths.rootResultFolder, 'Auto', 'CrossValidation')
        print Paths.rootResultFolder
    ...
    def createLanguageFolder():
        ...
        print Paths.langResultFolder
        if not os.path.exists(Paths.langResultFolder):
            os.makedirs(Paths.langResultFolder)

    @staticmethod
    def createMWELexic(dic, path = os.path.join(Paths.langResultFolder, 'Dictionary.md')):
    ...
    def createStaticParsingReports(sents, crossValidationIdx=''):
        ...
        printingPath = os.path.join(Paths.xpPath, 'StaticParsing' + str(crossValidationIdx) + '.md')
    ...
    def createEmbeddingSentsReports(sents, crossValidationIdx=''):
        ...
        sentsForPrinting = sorted(sentsForPrinting, key=lambda Sentence: len(Sentence.vMWEs), reverse=True)
        printingPath = os.path.join(Paths.xpPath, 'EmbeddedSents' + str(crossValidationIdx) + '.md')
        staticParsingFile = open(printingPath, 'w')
        result = ''
        for sent in sentsForPrinting:
            result += str(sent)
    ...
    def createParsingReport(testingSents, mweDictionary, crossValidationIdx=''):
        ...
        result = ''
        sentsForPrinting = sorted(sentsForPrinting, key=lambda Sentence: len(Sentence.vMWEs), reverse=True)
        printingPath = os.path.join(Paths.xpPath, 'Parsing' + str(crossValidationIdx) + '.md')
        ...
        printingPath = os.path.join(Paths.xpPath, 'Parsing' + str(crossValidationIdx) + '-long.md')
        parsingFile = open(printingPath, 'w')
        result = ''
        for sent in sentsForPrinting:
    ...
    def editTotalReadMe(fScore, recall, precision, corpus, testSents, identifiedIntellegentillyPrecent=.0,
                        identifiedSemiIntellegentillyPercent=.0, path=Paths.evalResultsFile):
    ...
    def createEvaluationFiles(sents):
        ...
        if not XPParams.useCrossValidation:
            if not XPParams.realExper:
                ...
                path = os.path.join(Paths.langResultFolder, Paths.languageName + '.gold')
                wFile = open(path, 'w')
            ...
            if XPParams.realExper:
                path = os.path.join(Paths.langResultFolder, 'test.system.parsemetsv')
            else:
                path = os.path.join(Paths.langResultFolder, Paths.languageName + '.pred')
            wFile = open(path, 'w')
